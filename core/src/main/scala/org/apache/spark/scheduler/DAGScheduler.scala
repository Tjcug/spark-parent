...
class DAGScheduler(
  ...
  def runJob[T, U](
    ...
    // which causes concurrent SQL executionDs to fail if a fork-join pool is used. Note that
      ...
      case Success =>
        ...
        task match {
          ...
          case smt: ShuffleMapTask =>
            ...
            // 如果当前调度阶段处在运行调度阶段列表中，并且没有任务处于挂起状态（均已完成），则标记
            // 该调度阶段已经完成并注册输出结果的位置
